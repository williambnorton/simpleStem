...
start_cmd() {
  case "$1" in
    ...
    heartbeat) # Write pid/alive state for every service to Drive every
               # ~20s ($DATA/.run-status/librarian.json) so BOTH machines'
               # dashboards show truthful green/red daemon cards with PIDs.
               echo "while true; do '$BASE/librarian_heartbeat.sh' || true; sleep 20; done" ;;
