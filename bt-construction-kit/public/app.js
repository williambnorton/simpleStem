...
const els = {
  ...
  lyricsSectionLabel: document.getElementById('lyrics-section-label'),
  ...
  lyricsShiftBack: document.getElementById('lyrics-shift-back'),
  lyricsShiftForward: document.getElementById('lyrics-shift-forward'),
...
async function pollIngestWatch() {
  ...
  for (const w of watch) {
    ...
    // Match a finished song against this submission. Three ways, fastest
    // first: (a) exact videoId field on the library row (added server-side
    // by scanStems' source_url parser); (b) source_url substring; (c) the
    // legacy folder-name substring kept for songs ingested before scanStems
    // emitted source_url. Once any match hits, the row turns green
    // ("in library") and the renderer drops it from the visible list
    // after the doneAt grace window (60s by default).
    else if (libSongs.some(s =>
        (w.videoId && s.videoId === w.videoId)
        || (w.videoId && (s.source_url || '').includes(w.videoId))
        || (w.videoId && (s.folderName || s.base || '').toLowerCase().includes(w.videoId.toLowerCase()))
    )) {
...
function loadSong(song, opts) {
  ...
  // Show / hide the Lyrics button + reset any open overlay since we just
  // switched songs. Bill's lyrics_shift localStorage key is keyed by
  // folderName so it'll re-load when he opens this song again.
...
// ─── Clip Library: read-only browser ─────────────────────────────────
//
// As of 2026-06-21 the App no longer fetches clips from URLs or trims
// them. That work moved out to the standalone Clip Librarian (see
// clip_librarian/README.md at the simpleStem root). The App reads
// CUSTOM_LOOPS/ on demand via /api/custom-loops/list and surfaces the
// files here with audition + delete buttons. The Sampler panel
// (setupSamplerPanel below) handles per-song clip-as-action wiring.
function setupUrlLoopPanel() {
  ...
  async function refreshCustomLoopsList() {
    ...
    if (!loops.length) {
      list.innerHTML = '<li class="url-loop-empty">No clips yet. Curate some via the Clip Librarian (clip_librarian/README.md).</li>';
      return;
    }
    list.innerHTML = loops.map(l => `
      <li class="url-loop-row-item" data-file="${escapeHtml(l.file)}">
        <button class="url-loop-play" title="Audition"><i data-lucide="play"></i></button>
        <span class="url-loop-fname">${escapeHtml(l.file)}</span>
        <span class="url-loop-size">${(l.size / 1024).toFixed(0)} KB</span>
        <button class="url-loop-del" title="Delete this clip"><i data-lucide="trash-2"></i></button>
      </li>
    `).join('');
    if (window.lucide) lucide.createIcons();
    list.querySelectorAll('.url-loop-row-item').forEach(row => {
      const file = row.dataset.file;
      row.querySelector('.url-loop-play').addEventListener('click', (e) => {
        e.stopPropagation();
        playCustomLoop(file);
      });
  ...
  // Public for the Sampler panel: when a new clip lands externally
  // (Drive sync, manual drop), refresh both lists.
  window._refreshCustomLoopsList = refreshCustomLoopsList;
...
/* ==========================================
   ...
   ========================================== */
// ── Lyrics overlay ────────────────────────────────────────────────────────
// Per-song lyric chunks aligned to sections. On the first click of the
// Lyrics button we fetch from Genius (if not cached in metadata.json yet),
// then auto-split the lyrics into one chunk per section on the lane. The
// overlay tracks the playhead and shows the chunk for the current section.
// "Shift" buttons move the chunk's start by N lines so Bill can hand-tune
// alignment if the auto-split is off. State persists per-song in
// localStorage as a shift offset (lines-per-chunk + start-line).
//
// Chunk model: chunks[i] is the lyrics block for section i (i.e. the
// time window between section i-1's end and section i's t). Generated by
// _rebuildLyricChunks() whenever lyrics or sections change.
const LYRICS_LINES_PER_CHUNK_DEFAULT = 5;   // Bill asked for 4-6
let lyricsState = {
  visible:        false,
  fetching:       false,
  songBase:       null,
  rawLines:       [],   // every non-blank line of the lyrics, sectioned-flat
  chunks:         [],   // [{label, text, startLine, endLine}], indexed by section
  shiftLines:     0,    // global shift in lines (Bill's tune knob)
};

function setupLyrics() {
  if (!els.btnLyrics) return;
  els.btnLyrics.addEventListener('click', toggleLyrics);
  els.lyricsHide.addEventListener('click', hideLyrics);
  els.lyricsShiftBack.addEventListener('click',    () => shiftLyricChunks(-1));
  els.lyricsShiftForward.addEventListener('click', () => shiftLyricChunks(+1));
}

// Show the Lyrics button only when a song is loaded that has stems
// (sections live on stems songs only). Called from loadSong / variant
// switches via a small observer below.
function refreshLyricsButton() {
  if (!els.btnLyrics) return;
  const hasSong = !!(currentSong && currentSong.type === 'stems');
  els.btnLyrics.style.display = hasSong ? '' : 'none';
  if (!hasSong) hideLyrics();
}

async function toggleLyrics() {
  if (lyricsState.visible) { hideLyrics(); return; }
  if (!currentSong || currentSong.type !== 'stems') return;
  const base = currentSong.folderName;
  // (Re)build from in-memory lyrics if we have them; otherwise fetch.
  if (!currentSong.lyrics_chunks && !currentSong.lyrics) {
    await fetchLyricsForCurrent();
    if (!currentSong.lyrics) return; // showSongLoadError-ish already alerted
  }
  // Pull persisted shift for this song so re-opens reuse Bill's hand-tune.
  try {
    const raw = localStorage.getItem(`simpleStem.lyricsShift.${base}`);
    lyricsState.shiftLines = raw ? parseInt(raw, 10) || 0 : 0;
  } catch (e) { lyricsState.shiftLines = 0; }
  _rebuildLyricChunks();
  _showLyricsOverlay();
}

async function fetchLyricsForCurrent() {
  if (lyricsState.fetching) return;
  const base = currentSong && currentSong.folderName;
  if (!base) return;
  lyricsState.fetching = true;
  if (els.btnLyrics) { els.btnLyrics.disabled = true; }
  try {
    const r = await fetch(`/api/song/${encodeURIComponent(base)}/fetch-lyrics`, { method: 'POST' });
    const data = await r.json().catch(() => ({}));
    if (!r.ok || !data.lyrics) {
      alert(`Couldn't fetch lyrics: ${data.error || r.statusText}\n${data.hint || ''}`);
      return;
    }
    // Patch the in-memory song so subsequent clicks don't re-fetch.
    currentSong.lyrics = data.lyrics;
    currentSong.lyrics_chunks = data.lyrics_chunks;
    // Also patch mergedLibrary so other surfaces (re-load via setlist) see it.
    try {
      mergedLibrary.forEach(m => {
        const sv = m.variants.find(v => v.type === 'stems' && v.folderName === base);
        if (sv) { sv.lyrics = data.lyrics; sv.lyrics_chunks = data.lyrics_chunks; }
      });
    } catch (e) {}
  } finally {
    lyricsState.fetching = false;
    if (els.btnLyrics) { els.btnLyrics.disabled = false; }
  }
}

// Flatten lyrics into non-blank lines, then chunk them across the song's
// sections. Each section gets ~5 lines (4-6 range Bill asked for) by
// equal division, with a small "swap" pass that merges very short chunks
// into the next so a 1-line tail doesn't strand a chorus.
function _rebuildLyricChunks() {
  lyricsState.chunks = [];
  lyricsState.rawLines = [];
  if (!currentSong || !currentSong.lyrics) return;
  const lines = currentSong.lyrics.split(/\r?\n/)
    .map(s => s.trim())
    .filter(s => s && !/^\[.*\]$/.test(s));   // drop [Verse 1] header lines
  lyricsState.rawLines = lines;
  const sections = (automationSections || []).slice().sort((a, b) => a.t - b.t);
  // Use sections + 1 for the implicit "outro" tail after the last marker.
  const slotCount = Math.max(1, sections.length + 1);
  // Aim for the user-friendly 4–6 lines/chunk. If the per-section quota
  // would push outside that range, just use the line target and accept
  // that the last section gets the remainder.
  const target = Math.max(4, Math.min(6, Math.round(lines.length / slotCount)));
  let cursor = lyricsState.shiftLines;
  for (let i = 0; i < slotCount; i++) {
    const label = (sections[i] && SECTION_COLORS[sections[i].color])
      ? SECTION_COLORS[sections[i].color].name : (i === slotCount - 1 ? 'Outro' : `Section ${i + 1}`);
    const startLine = Math.max(0, Math.min(lines.length, cursor));
    const endLine   = Math.min(lines.length, startLine + target);
    const text = lines.slice(startLine, endLine).join('\n') || '—';
    lyricsState.chunks.push({ label, text, startLine, endLine });
    cursor = endLine;
  }
}

function _showLyricsOverlay() {
  if (!els.lyricsOverlay) return;
  lyricsState.visible = true;
  els.lyricsOverlay.style.display = 'flex';
  _refreshLyricsForPlayhead();
}
...
// Pick the section index whose [prev.t, this.t] range contains `now`.
// Returns index into chunks[] (0..sections.length, where N = "outro" tail).
function _currentSectionIdx(now) {
  const sections = (automationSections || []).slice().sort((a, b) => a.t - b.t);
  for (let i = 0; i < sections.length; i++) {
    if (now <= sections[i].t) return i;
  }
  return sections.length;   // past the last marker → outro slot
}

function _refreshLyricsForPlayhead() {
  if (!lyricsState.visible || !lyricsState.chunks.length) return;
  const active = Object.values(audioElements).filter(ae => audioHasSrc(ae));
  const now = (active[0] && active[0].currentTime) || 0;
  const idx = Math.min(lyricsState.chunks.length - 1, _currentSectionIdx(now));
  const chunk = lyricsState.chunks[idx];
  els.lyricsSectionLabel.textContent = chunk.label;
  els.lyricsProgress.textContent = `${idx + 1} / ${lyricsState.chunks.length}`;
  els.lyricsBody.textContent = chunk.text;
}

// Manual realignment: shift the WHOLE alignment by ±1 line. Persists in
// localStorage per song so a re-open keeps Bill's tune.
function shiftLyricChunks(delta) {
  lyricsState.shiftLines = Math.max(-50, Math.min(50, (lyricsState.shiftLines || 0) + delta));
  try {
    if (currentSong && currentSong.folderName) {
      localStorage.setItem(`simpleStem.lyricsShift.${currentSong.folderName}`, String(lyricsState.shiftLines));
    }
  } catch (e) {}
  _rebuildLyricChunks();
  _refreshLyricsForPlayhead();
...
// Tick — re-pick the visible chunk as the playhead crosses section
// boundaries. Cheap (string compare per tick).
setInterval(_refreshLyricsForPlayhead, 250);
