...
// Shared desk-folder resolver (Bill 2026-07-14: "open the recently
// created folder, not the general STEMS parent"): exact base -> title
// match against the STEMS directory names (newest match wins) -> the
// newest STEMS folder if modified in the last 30 min (a just-queued
// render) -> STEMS root. All reads ASYNC with a 3 s bound — never a sync
// Drive read on the event loop (house policy).
async function resolveDeskFolder(base, title) {
  const stemsRoot = path.join(SIMPLE_STEM_ROOT, 'STEMS');
  if (base) {
    const s = safeSongDir(base);
    if (s && fs.existsSync(s.dir)) return { target: s.dir, how: 'exact' };
  }
    ...
    const best = await bounded((async () => {
      ...
      for (const e of entries) {
        ...
        if (score > bestScore || (score === bestScore && mt > bestMt)) {
          bestName = e.name; bestScore = score; bestMt = mt;
        }
      }
      return bestName;
    })());
  ...
  const newest = await bounded((async () => {
    const entries = await fs.promises.readdir(stemsRoot, { withFileTypes: true });
    let newestName = null, nm = 0;
    for (const e of entries) {
      if (!e.isDirectory()) continue;
      try {
        const mt = (await fs.promises.stat(path.join(stemsRoot, e.name))).mtimeMs;
        if (mt > nm) { nm = mt; newestName = e.name; }
      } catch (er) {}
    }
    return (newestName && Date.now() - nm < 30 * 60 * 1000) ? newestName : null;
  })());
  if (newest) return { target: path.join(stemsRoot, newest), how: 'newest' };
  return { target: stemsRoot, how: 'root' };
}
...
// Honest hover peek (Bill 2026-07-14: "I want to see the file sizes as I
// believe they are empty, and the full path"): resolves the same way as
// reveal and returns the REAL directory listing with byte sizes.
app.get('/api/desktop/inspect', async (req, res) => {
  try {
    const base = String(req.query.base || '').trim();
    const title = String(req.query.title || '').trim();
    const { target, how } = await resolveDeskFolder(base, title);
    if (how === 'root') return res.json({ ok: false, how, path: target });
    const listing = await Promise.race([(async () => {
      const entries = await fs.promises.readdir(target, { withFileTypes: true });
      const files = [];
      for (const e of entries.slice(0, 40)) {
        let size = null, mtime = null;
        try {
          const st = await fs.promises.stat(path.join(target, e.name));
          size = e.isDirectory() ? null : st.size;
          mtime = st.mtimeMs;
        } catch (er) {}
        files.push({ name: e.name + (e.isDirectory() ? '/' : ''), size, mtime });
      }
      files.sort((a, b) => a.name.localeCompare(b.name));
      return files;
    })(), new Promise(r => setTimeout(() => r(null), 3000))]);
    res.json({ ok: true, how, path: target, files: listing || [] });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
